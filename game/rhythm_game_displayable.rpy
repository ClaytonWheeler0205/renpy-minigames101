screen rhythm_game(audio_path, beatmap_path, difficulty):
    default rhythm_game_displayable = RhythmGameDisplayable(audio_path, beatmap_path, difficulty)
    
    add Solid('#000')
    add rhythm_game_displayable

    # show the score heads-up display (HUD)
    text 'Great: ' + str(rhythm_game_displayable.great_hits) + '\n' + 'Good: ' + str(rhythm_game_displayable.num_hits):
    ...
    if rhythm_game_displayable.has_ended:
        ...
        timer 2.0 action Return(
            ((rhythm_game_displayable.num_hits + rhythm_game_displayable.great_hits), rhythm_game_displayable.num_notes)    
            )
...
init python:
    ...
    class RhythmGameDisplayable(renpy.Displayable):
        ...
        def __init__(self, audio_path, beatmap_path, difficulty):
            ...
            # the first st
            # an offset is neessary because there might be a delay between when the
            # displayable first appears on screen and the time the music starts playing
            # seconds, same unit as st, shown t
            ...
            # this scroll time is also the note's entire lifespan before it's either a
            ...
            self.note_offset = 3.0 / difficulty
            # speed = distance / time
            ...
            # map onset timestamp to whether it has been hit,
            self.onset_hits = {
            onset: False for onset in self.onset_times
            }
            self.num_hits = 0
            self.great_hits = 0
            # if the note is hit within 0.3 seconds of its actual onset
            # we consider it a hit
            # can set different threshold for Good, Great hit scoring
            self.hit_threshold = 0.3 / difficulty # seconds
            self.great_threshold = 0.2 / difficulty # seconds
            ...
            if ev.type == pygame.KEYDOWN:
                ...
                if not ev.key in self.keycode_to_track_idx:
                    if ev.key == pygame.K_p:
                        ...
                        renpy.music.set_pause(self.is_paused)
                        if self.is_paused:
                            self.old_st = st
                        else:
                            self.time_offset += (st - self.old_st)
                    return
                # loop up the track that corresponds to the key pressed
                ...
                for onset, _ in active_notes_on_track:
                    ...
                    if abs(curr_time - onset) <= self.hit_threshold and not self.onset_hits[onset]:
                        self.onset_hits[onset] = True
                        if abs(curr_time - onset) < self.great_threshold:
                            self.great_hits += 1
                        else:
                            self.num_hits += 1
                        # redraw immediately because not the note should disappear from
                        renpy.redraw(self, 0)
                        # refresh the screen
                        renpy.restart_interaction()
        ...
        def get_active_notes_per_track(self, curr_time):
            active_notes = {
                track_idx: [] for track_idx in range(self.num_track_bars)
            }
            ...
            for onset, track_idx in zip(self.onset_times, self.random_track_indices):
                ...
                time_before_appearance = onset - curr_time
                ...
                # break out of the loop so we don't process subsequent notes
                elif time_before_appearance > self.note_offset:
                    break
